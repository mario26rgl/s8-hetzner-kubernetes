locals {
  ...
  autoscaler_yaml = length(var.autoscaler_nodepools) == 0 ? "" : templatefile(
    ...
    {
      ca_image                                   = var.cluster_autoscaler_config.image
      ca_version                                 = var.cluster_autoscaler_config.image_tag
      ca_replicas                                = var.cluster_autoscaler_config.replicas
      ca_resource_limits                         = var.cluster_autoscaler_config.resource_limits
      ca_resources                               = var.cluster_autoscaler_config.resource_values
      cluster_autoscaler_extra_args              = var.cluster_autoscaler_config.extra_args
      cluster_autoscaler_log_level               = var.cluster_autoscaler_config.log_level
      cluster_autoscaler_log_to_stderr           = var.cluster_autoscaler_config.log_to_stderr
      ...
      enable_ipv4                                = !(var.cluster_autoscaler_config.disable_ipv4 || local.use_nat_router)
      enable_ipv6                                = !(var.cluster_autoscaler_config.disable_ipv6 || local.use_nat_router)
  })
...
resource "terraform_data" "configure_autoscaler" {
  ...
  connection {
    user           = "root"
    private_key    = var.ssh_private_key # null
  ...
  part {
    ...
    content = templatefile(
      "${path.module}/templates/autoscaler-cloudinit.yaml.tpl",
      {
        ...
        k3s_config = yamlencode(merge(
          {
            ...
            # Kubelet arg precedence (first wins): local.kubelet_arg > nodepool.kubelet_args > k3s_global_kubelet_args > k3s_autoscaler_kubelet_args
            kubelet-arg = concat(
              local.kubelet_arg,
              var.autoscaler_nodepools[count.index].kubelet_args,
              var.k3s_global_kubelet_args,
              var.k3s_autoscaler_kubelet_args
            )
        ...
        private_network_only         = var.cluster_autoscaler_config.disable_ipv4 && var.cluster_autoscaler_config.disable_ipv6,
...
# Updates the list of registries on autoscaled nodes
...
# Applies custom kubelet configuration
